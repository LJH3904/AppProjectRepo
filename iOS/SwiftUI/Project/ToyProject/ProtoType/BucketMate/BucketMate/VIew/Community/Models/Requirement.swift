...
struct RecruitPost: Identifiable {
    let id = UUID()
    var user: User
...
struct Requirement: Identifiable {
    ...
    var predictExpense: String {
    return "\(Int(minBudget) / 10000) ~ \(Int(maxBudget) / 10000)만원"
    }
}

enum Gender: String, CaseIterable {
    case onlyMan = "남자만", onlyWonam = "여자만", both = "남녀혼성", dontCare = "상관없음", none = ""
